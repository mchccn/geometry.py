...
class Vector4:
  ...
  def __init__(self, x, y, z, w):
    self.__coords = (x if x != None else 0, y if y != None else 0, z if z != None else 0, w if w != None else 0)
    
  def __iter__(self):
    yield self.__coords[0]
    yield self.__coords[1]
    yield self.__coords[2]
    yield self.__coords[3]
    
  def add(self, thing, y, z, w):
    if isinstance(thing, Vector4):
      self.x += thing.x
      self.y += thing.y
      self.z += thing.z
      self.w += thing.w
      ...
      self.z += w if w != None else thing
      
    return self
  
  def subtract(self, thing, y, z, w):
    if isinstance(thing, Vector4):
      self.x -= thing.x
      self.y -= thing.y
      self.z -= thing.z
      ...
      self.y -= y if y != None else thing
      self.z -= z if z != None else thing
      self.w -= w if w != None else thing
      
    return self
  
  def multiply(self, thing, y, z, w):
    if isinstance(thing, Vector4):
      self.x *= thing.x
      self.y *= thing.y
      self.z *= thing.z
  ...
  @staticmethod
  def is_vector4(thing):
    return isinstance(thing, Vector4)
